...
# If clip_per_dim = True and real_data = False:
#     We show that clipping per dimension leads to the compressed matrices
#     have the same overlap vs. precision curves, regardless of the decay speed.
# If clip_per_dim = False and real_data = False:
...
def eigenspace_overlap_vs_prec_n_d(vary_n,vary_d,prec_x,plot_bounds):
    ...
    if prec_x:
        x_str = 'prec'
        bs = [1,2,4,8,16]
        latexify_config['title'] = r'Eigen. Overlap ($\mathcal{E}$) vs. Precision ($b$)'
        latexify_config['xlabel'] = 'Precision ($b$)'
        latexify_config['xtick_pos'] = bs
        latexify_config['xtick_label'] = bs
        latexify_config['xlim'] = [1,16]
    else:
        assert vary_n or vary_d
        x_str = 'd' if vary_d else 'n'
        bs = [1,2,4]
        latexify_config['xlim'] = [None,None]
        latexify_config['xtick_pos'] = None
        latexify_config['xtick_label'] = None
        latexify_config['xlabel'] = 'Emb. Dim. ($d$)' if vary_d else 'Vocab. Size ($n$)'
        latexify_config['title'] = r'Eigen. Overlap ($\mathcal{E}$) vs. ' + latexify_config['xlabel']
        # if vary_d:
        #     latexify_config['xtick_pos'] = ds
        #     latexify_config['xtick_label'] = ds
        # else:
        #     latexify_config['xtick_pos'] = ns
        #     latexify_config['xtick_label'] = ns

    ax = latexify_setup_fig(latexify_config)
    full_matrices = False
    overlaps = np.zeros((len(ns),len(ds),(len(bs))))
    bounds = np.zeros((len(ns),len(ds),(len(bs))))
    a_values = np.zeros((len(ns),len(ds)))
    ...
    for i_n,n in enumerate(ns):
        for i_d,d in enumerate(ds):
            ...
            # s_max = S[0]
            s_min = S[d-1]
            a_values[i_n,i_d] = s_min / np.sqrt(n/d)
            print('a = {}'.format(a_values[i_n,i_d]))
        ...
        if vary_d:
            ...
            for i_n,n in enumerate(ns):
                for i_b,b in enumerate(bs):
                    ...
                    plt.plot(ds, overlaps[i_n,:,i_b], styles[i_b], label='$b={}$'.format(b))
                    # plt.plot(ds, overlaps[i_n,:,i_b], n_colors[i_b] + b_markers[i_b] + bound_line_styles[0])
                    # plt.plot(ds, overlaps[i_n,:,i_b], label='$b$='+str(b))
                    if plot_bounds:
            ...
            for i_d,d in enumerate(ds):
                for i_b,b in enumerate(bs):
                    ...
                    plt.plot(ns, overlaps[:,i_d,i_b], styles[i_b], label='$b={}$'.format(b))
                    # plt.plot(ns, overlaps[:,i_d,i_b], d_colors[i_b] + b_markers[i_b] + bound_line_styles[0])
                    # plt.plot(ns, overlaps[:,i_d,i_b], label='$b$='+str(b))
                    if plot_bounds:
...
def eigenspace_overlap_vs_a_multiple_prec_decay(plot_bounds):
    ...
    latexify_config['xtick_label'] = None
    latexify_config['xlim'] = [None,None]
    latexify_config['xlabel'] = '$a$'
    latexify_config['ylabel'] = r'$1-\mathcal{E}$'
    latexify_config['title'] = r'Eigen. Overlap ($\mathcal{E}$) vs. $a$'
    latexify_config
    ax = latexify_setup_fig(latexify_config)
    full_matrices = False
    overlaps = np.zeros((len(bs),len(decays)))
    bounds = np.zeros((len(bs),len(decays)))
    a_values = np.zeros(len(decays))
    lim = 1.0/np.sqrt(d)
    X_orig = np.random.uniform(low=-lim, high=lim, size=(n,d))
    for i_d,decay in enumerate(decays):
    ...
    for i_b,b in enumerate(bs):
        ...
        plt.plot(a_values, overlaps[i_b,:], styles[i_b], label='$b={}$'.format(b))
        if plot_bounds:
            leg.append('bound (b={})'.format(b))
...
# Show effect of picking the clipping value separate in each direction
# after rotating onto the basis of eigenvectors, relative to the normal
# uniform quantization approach (single clipping value for whole matrix).
def eigenspace_overlap_glove_clip_per_dim(logy):
    ...
    X,_ = utils.load_embeddings('C:\\Users\\avnermay\\Babel_Files\\smallfry\\base_embeddings\\glove10k\\glove.6B.300d.10k.txt')
    n,d = X.shape
    adaptive=True
    U,S,_ = np.linalg.svd(X,full_matrices=full_matrices)
    X_rotate = U * S
    ...
    plt.figure(1)
    plt.subplot(411)
    plt.plot(np.max(X_rotate,axis=0))
    plt.plot(np.min(X_rotate,axis=0))
    plt.plot(np.max(X,axis=0))
    plt.plot(np.min(X,axis=0))
    plt.legend([
        'X_rotate (max)',
        'X_rotate (min)',
        'X (max)',
    ...
    plt.subplot(413)
    for i in range(4):
        plt.hist(X_rotate[:,i].flatten(),density=True)
    plt.hist(X_rotate[:,90:100].flatten(),density=True)
    # plt.hist(X_rotate[:,290:300].flatten(),density=True)

    plt.subplot(414)
    plt.hist(X[:,0:10].flatten(),density=True)
    plt.hist(X[:,90:100].flatten(),density=True)
    plt.hist(X[:,290:300].flatten(),density=True)
    plt.show()
