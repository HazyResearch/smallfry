...
\section{Story}
\begin{enumerate}
	\item We present a simple method for compressing word embeddings using uniform quantization, and show that it attains strong performance at large compression ratios on a variety of tasks.
	It performs quite similarly to the more complex baselines (DCA, k-means), and much better than simple baselines (dim. reduction).
	\item We prove that, in the case of a bounded embedding matrix $X$, the Gram matrix of the quantized matrix $\tX$ is a $(\Delta_1,\Delta_2)$-spectral approximation of the Gram matrix of $X$, and thus enjoys good generalization bounds.
\end{enumerate}
...
\section{Theory}
\begin{enumerate}
	\item If the smallest non-zero singular value of $X$ is large, using a large regularizer does not affect generalization performance much.
	\item If we consider a $b$-bit uniform quantization $\tX$ of a bounded matrix $X$, $\tX\tX^T + \lambda I$ is a $(\Delta_1,\Delta_2)$-spectral approximation of $XX^T + \lambda I$ with high-probability, for $\Delta_1$ and $\Delta_2$ which decay exponentially fast in $2^b \lambda$.
\end{enumerate}
\subsection{Theory Validation}
\begin{enumerate}
	\item Show in experiments that a large smallest singular value does in fact results in large regularizers attaining good performance.
	\item Show that $2^b \lambda$ is quite predictive of $\Delta_1$ and $\Delta_2$.
	\item Show that choosing the right clipping value is really about trading off small $\Delta_1$ vs. $\Delta_2$.
	Clipping to a very small value results in large $\Delta_1$, but small $\Delta_2$.
	Clipping to a large value results in small $\Delta_1$, but large $\Delta_2$.
	\item Show on synthetic data that $(\Delta_1,\Delta_2)$ are much more predictive of generalization performance than $\|XX^T - \tX\tX^T\|_F$.
\end{enumerate}

\section{Experiments}
\begin{enumerate}
	\item Large-scale experiments: show our method does similarly to k-means and DCA (on Glove and FastTest), and better than dimensionality reduction (Glove), on a variety of tasks (intrinsics, sentiment, DrQA).
	\item Show that given a fixed memory budget, using low-precision and many dimensions is better than using high-precision with fewer dimensions.  Show there is a ``sweet spot''.
	\item Show that $(\Delta_1,\Delta_2)$ are predictive of generalization performance (better than PIP loss $\|XX^T - \tX\tX^T\|_F$) on real tasks.
\end{enumerate}
...
\begin{enumerate}
	...
	We can also provide a supervised explanation for this phenomenon (adding more dimensions reduces this bias but increases the variance of the kernel ridge regression model).
...
\begin{enumerate}
	\item Method
	\begin{enumerate}
		\item Should we use $(\Delta_1,\Delta_2)$ to choose the clipping point?
		\item Deterministic vs. stochastic?
	\end{enumerate}
	\item Theory
	\begin{enumerate}
		\item Can we account for the clipping operation in the theory?  Currently we assume matrix is clipped, and thus do not consider the effect of clipping on $(\Delta_1,\Delta_2)$.
	\end{enumerate}
	\item Theory validation
	\begin{enumerate}
		\item It would be good to show that ($\Delta_1$,$\Delta_2$) correlates better with generalization performance than $\|XX^T - \tX \tX^T\|_F$ (this is the PIP loss from the recent NeurIPS paper).
	\end{enumerate}
\end{enumerate}
