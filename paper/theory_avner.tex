...
combining this observation with the above theoretical results helps explain the strong performance of the uniformly quantized embeddings, even at very low precisions. \todo{Is this the right place for this empirical observation?}
...
Then, for various values of $\sigma$, and various values of $\lambda$, we learn the linear ridge regression model $w = ( X^T X + \lambda I_d)^{-1}X^Ty$, and measure the means-squared error $\|Xw-y\|_2^2$

%we consider a simple linear model $y=w^T x$, and measure how the generalization error as the ratio $a\defeq grows as the regularization parameter $\lambda$ grows, 
