...
\todo{New tensor paper? \citep{tensor19}.}
%\todo{more concise?}
...
%For example, using the DrQA model \citep{drqa17} on the Stanford Question Answering Dataset  \citep{squad16}, uniform quantization attains a compression ratio of 32x with an average F1 score only 0.47\% absolute below the uncompressed GloVe embeddings, while the state-of-the-art deep compositional code learning method \cite{dccl17} is 0.43\% below. \todo{should we keep the example?}. 
...
In particular, we observe that the reconstruction error of the embeddings, the Pairwise Inner Product (PIP) loss \citep{yin18}, and two spectral measures of approximation between the pairwise inner product (Gram) matrices of the embeddings \citep{avron17,lprff18} all \todo{correlate poorly} with the downstream performance of the compressed embeddings;
for example, across a large number of compressed GloVe embeddings, these metrics attain Spearman rank correlation coefficients of \todo{WW,XX,YY,ZZ} respectively with respect to question answering performance on the Stanford Question Answering Dataset (SQuAD) \citep{squad16}.
...
Toward this end, we introduce the \textit{eigenspace overlap metric} which we show \todo{correlates strongly} with the downstream performance of compressed embeddings across a variety of tasks, and prove a generalization bound for the compressed embeddings in terms of this metric in the context of linear regression.
On the above-mentioned question answering task, for example, this metric attains a Spearman rank correlation coefficient of \todo{AA}.
...
%Intuitively, this is a natural metric because a linear regressor is only able to capture the part of the label vector in the span of the eigenvectors of the nonzero eigenvalues of the Gram matrix \todo{(too low-level/technical?)}.
...
In Section~\ref{sec:challenge} we empirically compare the various compression methods, and show that the existing metrics \todo{do not align well} with downstream performance across different compressed embedding types.
To address this \todo{poor alignment}, we introduce the eigenspace overlap metric in Section~\ref{sec:new_metric}, along with corresponding generalization bounds and experiments.
